...
- 已在 bridge 封装收敛后刷新基线（`2026-02-19`）：
  ...
  - stdlib 静态 bridge 引用：`99`（覆盖 `18` 个模块，较 `226` / `26` 下降），
  - `tests_flv` 审计 bridge 调用：`640`（pure_call `564`、rpc `66`、call `10`，较 `1711` 下降）。
- 在 `stringlib` 新增可复用的低层字符串封装，并将核心模块（`url`、`path`、`datetime`、`csv`、`cliargs`、`regex`、`httplib.core`、`struct`）迁移到该封装，降低对 bridge 的直接耦合。
